// GitHub owner/repo for releases API — set your fork (or set env in shell before packaging).
var SILENTGUARD_GITHUB_REPO = 'OptimusFaber/SilentGuard';
...
var resp = new HTTPResponse("https://api.github.com/repos/" + SILENTGUARD_GITHUB_REPO + "/releases");
var data;
var resp_error;
if (!resp.error) {
    try {
...
if (resp_error) {
    ...
    if (winget_package) {
        bound = getLatestWingetVersion('OptimusFaber.SilentGuard');
    }
            ...
            if (!winget_package) {
                ...
                if (chocolatey_package) {
                    let nupkg_errors = download(
                        "https://github.com/" + SILENTGUARD_GITHUB_REPO + "/releases/download/" +
                        latest_tag_name + "/silentguard." + latest_tag_name + ".nupkg",
                        "downloads/silentguard." + latest_tag_name + ".nupkg", true);
                    if (nupkg_errors == '') {
                ...
                archive_name = 'OptimusFaber.SilentGuard';
            }
            if (errors == '') {
                let index2 = ask(
                ...
                if (index2 == 1) {
                    ...
                    try {
                        ...
                        if (object_keys.length > 0) {
                            ...
                            updater_args.push('silentguard'); //NKR_SOFTWARE_NAME
                        }
                        is_newer = true;
                    } catch (e) {
                        info(e);
