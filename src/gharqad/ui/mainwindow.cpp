...
MainWindow::MainWindow(QWidget *parent)
  ...
  // software_name
  {
    ...
    if (software_name.trimmed() == "") {
      software_name = globalSettings.value("program_name", "SilentGuard").toString();
    }
    software_core_name =
  ...
  //
  {
    ...
    if (!dashboardDir.exists("index.html")) {
      if (auto dashFile = QFile(":/systray/dashboard-notice.html");
...
bool isNewer(QString assetName) {
  if (QString(NKR_VERSION).isEmpty())
    return false;
  //  assetName = assetName.mid(8); // take out silentguard-
  QString version;
...
void MainWindow::CheckUpdate(bool button_clicked) {
  ...
  *b = false;
  ...
  QString archive_name = "silentguard.zip",
#ifdef SKIP_JS_UPDATER
...
skip1:
  ...
  {
    auto resp = NetworkRequestHelper::HttpGet(
        silentguardGitHubReleasesApiUrl());
    if (!resp.error.isEmpty()) {
      runOnUiThread([=, this] {
