!ifdef SOFTWARE_NAME
    Name "${SOFTWARE_NAME}"
!else
	!define SOFTWARE_NAME SilentGuard
    Name "SilentGuard"
!endif
...
!ifdef OUTFILE
    OutFile "${OUTFILE}.exe"
!else
    OutFile "silentguard_setup.exe"
!endif
InstallDir $WINDIR\SilentGuard\.__installer_no_default__
...
VIAddVersionKey "CompanyName" "OptimusFaber"
VIAddVersionKey "FileDescription" "Installer for ${SOFTWARE_NAME}"
VIAddVersionKey "LegalCopyright" "© 2026 OptimusFaber"
...
Function .onInit
	...
	${Else}
		StrCpy $isAdmin "1"
	${EndIf}
	${If} "$INSTDIR" != "$WINDIR\SilentGuard\.__installer_no_default__"
		
	${Else}
		${If} "$isAdmin" == "1"
			${If} ${RunningX64}
				StrCpy $INSTDIR "$PROGRAMFILES64\SilentGuard"
			${Else}
				StrCpy $INSTDIR "$PROGRAMFILES\SilentGuard"
			${EndIf}
		${Else}
			StrCpy $INSTDIR "$APPDATA\SilentGuard"
		${EndIf}
	${EndIf}
...
!define MUI_ICON "res\silentguard.ico"
...
!define MUI_FINISHPAGE_RUN "$INSTDIR\silentguard.exe"
...
Section "Install"
  ...
  !ifdef DIRECTORY
    File /r  "${DIRECTORY}\silentguard_core.exe"
  !else
    File /r  ".\deployment\windows64\silentguard_core.exe"
  !endif

  ${If} "$VCRedistNeeded" != "1"
    nsExec::ExecToLog '"$INSTDIR\$RandomGUID\silentguard_core.exe" -installer-mode -kill-processes "$INSTDIR" -ignore-pid "$PID" -ignore-pid "$PPID" -ignore-pid "$PPPID" '
  ${Else}
    nsExec::ExecToLog '"$INSTDIR\$RandomGUID\silentguard_core.exe" -installer-mode -kill-processes "$INSTDIR" -vcredist-install -ignore-pid "$PID" -ignore-pid "$PPID" -ignore-pid "$PPPID" '
  ${EndIf}
  
  !insertmacro MoveFile "$INSTDIR\$RandomGUID\silentguard_core.exe" "$INSTDIR\silentguard_core.exe" 
;  ${PowerShellExec} "\
;    Write-Host $\"=> $INSTDIR$\" ;				\
...
;	"
  ...
  !ifdef DIRECTORY
    File /r /x silentguard_core.exe "${DIRECTORY}\*"
  !else
    File /r /x silentguard_core.exe ".\deployment\windows64\*"
  !endif
  ${Else}
  !ifdef DIRECTORY
    File /r  "${DIRECTORY}\*"
  ...
  ${If} "$UnpackOnly" != "1"
    ...
    CreateShortcut "$desktop\${SOFTWARE_NAME}.lnk" "$INSTDIR\silentguard.exe" "" "$INSTDIR\silentguard.exe" 0
    CreateShortcut "$SMPROGRAMS\${SOFTWARE_NAME}.lnk" "$INSTDIR\silentguard.exe" "" "$INSTDIR\silentguard.exe" 0

    !insertmacro RegString "Software\silentguard" "InstallPath" "$INSTDIR"
    !insertmacro RegString "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "DisplayName" "${SOFTWARE_NAME}"
	!insertmacro RegString "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "DisplayIcon" "$INSTDIR\silentguard.exe"
    !insertmacro RegString "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "UninstallString" "$INSTDIR\uninstall.exe"
    !insertmacro RegString "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "InstallLocation" "$INSTDIR"
    !insertmacro RegString "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "Publisher" "SilentGuard"
	${GetSize} "$INSTDIR" "/S=0K" $0 $1 $2
    !insertmacro RegInteger "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "EstimatedSize" $0
    !insertmacro RegInteger "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "NoModify" 1
    !insertmacro RegInteger "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard" "NoRepair" 1
...
Section "Uninstall"
  ...
  RMDir "$SMPROGRAMS\SilentGuard"

  RMDir /r "$INSTDIR"

  Delete "$INSTDIR\uninstall.exe"

  !insertmacro DropReg "Software\silentguard"
  !insertmacro DropReg "Software\Microsoft\Windows\CurrentVersion\Uninstall\silentguard"
