...
rm -Rf silentguard
cp -Rf silentguard_chocolatey silentguard

(
cd silentguard

sed -i "s~@URL_x86@~${url_x86}~g;s~@URL_x64@~${url_x64}~g;s~@SHA_x86@~${sha_x86}~g;s~@SHA_x64@~${sha_x64}~g;" tools/chocolateyinstall.ps1
sed -i "s~<version>.*</version>~<version>${INPUT_VERSION}</version>~g;" silentguard.nuspec
...
then
  ...
  choco push silentguard*.nupkg --source 'https://push.chocolatey.org/'
fi
if [[ "${UPLOAD_WITH_GH}" == 'yes' ]]
then
  gh release upload "${INPUT_VERSION}" silentguard*.nupkg --clobber
fi
)
