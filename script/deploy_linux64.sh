...
[[ -f "$BUILD/silentguard_core" ]] && cp "$BUILD/silentguard_core" "$DEST"
[[ -f "$BUILD/updater" ]] && cp "$BUILD/updater" "$DEST"

#### copy icon ####
cp ./res/silentguard.ico "$DEST/silentguard.ico"
...
(
cd "$CURDIR"
"./linuxdeploy-$ARCH2.AppImage" --appdir $DEST  --executable $DEST/silentguard --plugin qt
)
...
cd "$DEST"
patchelf --set-rpath '$ORIGIN/usr/lib' ./silentguard

shopt -s extglob

rm -rfv silentguard_directory
mkdir silentguard_directory
mv !(silentguard_directory) silentguard_directory
mv silentguard_directory silentguard
tar -czvf $DEPLOYMENT/$version_standalone-linux-${ARCH}.tar.gz silentguard
...
cp -Rfv ../silentguard/!(updater|appimage|silentguard-${ARCH1}.AppImage) ./AppDir
(
cd AppDir
mv silentguard_core .silentguard_core_binary_file
cat << 'EOF' > silentguard_core
#!/bin/sh
exec "$(dirname $0)"/".silentguard_core_binary_file" -argv0 "${APPIMAGE}" "${@}"
EOF
cat << 'EOF' > AppRun
#!/bin/sh
export NEKOBOX_APPIMAGE_CUSTOM_EXECUTABLE="${NEKOBOX_APPIMAGE_CUSTOM_EXECUTABLE:-silentguard}"
